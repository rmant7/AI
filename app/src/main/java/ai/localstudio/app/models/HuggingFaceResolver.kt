...
object HuggingFaceResolver {
    ...
    fun resolveAny(
        repoIds: List<String>,
        token: String? = null,
        quantPriority: List<String> = ArtifactResolver.DEFAULT_QUANT_PRIORITY,
    ): Pair<String, ResolvedModelFile> {
        val failures = mutableListOf<String>()
        for (repoId in repoIds) {
            try {
                return repoId to resolveWithRetry(repoId, token, quantPriority)
     ...
     */
    private fun resolveWithRetry(repoId: String, token: String?, quantPriority: List<String>): ResolvedModelFile {
        var lastNetworkError: UnknownHostException? = null
        repeat(RESOLVE_RETRIES) { attempt ->
            try {
                return resolve(repoId, token, quantPriority)
    ...
    fun resolve(
        ...
        val best = ArtifactResolver.pickBest(entries, quantPriority = quantPriority)
