...
**[последняя сборка](https://github.com/rmant7/AI/releases)** (6 МБ, Android 8.0+,
подпись debug-ключом). Каждая сборка перед публикацией прогоняется на эмуляторе:
чат отвечает, экран Models разбирает каталог и ранжирует его под память
устройства.
...
Плюс инструментальные тесты на эмуляторе в CI: приложение запускается,
отправленный вопрос проходит роутер, пайплайн, контекст и память и возвращает
ответ; экран Models разбирает каталог и ранжирует его под память устройства.
Собранный APK — не «скомпилировалось», а «запускается и отвечает».
